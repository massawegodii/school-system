...
def add_student2(request):
    if request.method == 'POST':
        ...
        form = ModuleForm()
    return render(request, 'teacher_template/add_student2.html', {'form': form})

def display_module(request):
    modules = Module.objects.all()
    return render(request, 'student_template/display_module.html', {'modules': modules})   
...
def add_student2(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            row = [student.name, student.reg_number, student.coursework, student.final_result]
            worksheet.append(row)

            # Save the changes to the Excel file
            workbook.save('students.xlsx')

            return render(request, 'teacher_template/add_student2.html')
    else:
        form = ModuleForm()
...
worksheet = workbook.active
worksheet.append(['Name', 'Reg Number', 'Coursework', 'Final Result'])
workbook.save('students.xlsx')
 '''
...
def course_form(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            # Redirect to the second template
            return redirect('student_home')
    else:
        form = CourseForm()
    return render(request, 'admin_template/add_module.html', {'form': form})



def display_course(request):
    courses = Course.objects.all()
    return render(request, 'teacher_template/display_course.html', {'courses': courses})
...
#     context = {'page_title':'add module'}
#     if request.method == 'POST':
#         m_name = request.POST.get('module_name')
#         c_id = request.POST.get('course_code')
#         c_work = request.POST.get('course_work')
#         f_mark = request.POST.get('final_mark')
#         cred = str(request.POST.get('credit'))
        
        
#         sub = Subject.objects.filter(course_code = c_id).first()
#         dep = Dept.objects.filter(dept_id = m_name).first()
...
#             messages.success(request, "Module Successfully Added")
#         else:
#             if dep == None:
#                 messages.error(request, " %s Dept is not registered. Register The Department First From Here"%(m_name))
#                 return redirect('student_sub_register')
#             messages.error(request, "Could'nt add Module.. Subject is Alraeady registered")
  
#     return render(request, 'admin_template/add_module.html',context)

# def student_sub_register(request):
#     context = {'page_title':'View module'}
#     if request.method == 'POST':
#         m_name = request.POST.get('module_name')
#         c_id = request.POST.get('course_code')
#         c_work = request.POST.get('course_work')
...
def add_j(request):
    ...
    if request.method == 'POST' and request.FILES['myfile']:
        ...
        module_dir = 'C:\\Users\GODFREY MASSAWE\\Desktop\\PROJECT\\university_management System\\media'
